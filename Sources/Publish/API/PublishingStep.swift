...
public extension PublishingStep {
    ...
    static func addMarkdownFiles(at path: Path = "Content", customContentParser: ((String) -> String)? = nil) -> Self {
        step(named: "Add Markdown files from '\(path)' folder") { context in
            let folder = try context.folder(at: path)
            try await MarkdownFileHandler().addMarkdownFiles(in: folder, to: &context, customContentParser: customContentParser)
        }
    }
